...
module PRISM =
    ...
    let mappings =
        [
            ...
            // heart rate
            {
                ...
                HighPoints = 4
                LowRange = 215.
                MidRangeLow = Some 215.
                MidRangeHigh = Some 225.
                HighRange = 225.
            }
            {
                Item = HeartRate
            ...
            }
            {
                ...
                HighPoints = 4
                LowRange = 185.
                MidRangeLow = Some 185.
                MidRangeHigh = Some 205.
                HighRange = 205.
            }
            {
                Item = HeartRate
                ...
                HighPoints = 4
                LowRange = 145.
                MidRangeLow = Some 145.
                MidRangeHigh = Some 155.
                HighRange = 155.
            }
            // creatinine
            {
            ...
            // pHl
            {
                ...
                HighPoints = 3
                LowRange = 7.47
                MidRangeLow = Some 7.48
                MidRangeHigh = Some 7.55
                HighRange = 7.55
            }
            // tCO2
            {
                ...
                HighPoints = 4
                LowRange = 34.0
                MidRangeLow = Some 34.0
                MidRangeHigh = Some 34.0
                HighRange = 34.0
            }
            // pCO2
            {
                ...
                HighPoints = 3
                LowRange = 50.0
                MidRangeLow = Some 50.0
                MidRangeHigh = Some 75.0
                HighRange = 75.0
            }
            // pAO2
            {
                ...
                HighPoints = 0
                LowRange = 42.0
                MidRangeLow = Some 42.0
                MidRangeHigh = Some 49.9
                HighRange = 49.9
            }
            // glu
            {
            ...
            // wbc
            {
                ...
                HighPoints = 5
                LowRange = 50.
                MidRangeLow = Some 100.
                MidRangeHigh = Some 200.
                HighRange = 200.
            }
        ]
    ...
    let calculators: (Item * ItemCalculator) list =
        ...
        let inline calc i a v f =
            ...
            |> function
                | [ m ] -> f v m
                | _ -> $"no mapping for %A{i} %A{a}" |> failwith
            |> fun s ->
                if i = MentalStatus || i = Pupils then
                    s |> Neuro
                else
                    s |> NonNeuro
        ...
        let calcOne i a v f =
            ...
            | TwoValues _ -> $"expected one value but got %A{v}" |> failwith
        // calculator for two values
        let calcTwo i a v f =
            match v with
            | NoValue -> 0 |> NonNeuro
            | OneValue v -> $"expected two values but got %A{v}" |> failwith
            | TwoValues(v1, v2) -> calc i a (v1, v2) f
        // generic calculator
        let genCalc v m =
            if v > m.HighRange then m.HighPoints else 0
        ...
        [
            ...
            fun a v ->
                fun v m ->
                    if v >= m.LowRange && v <= m.HighRange then
                        m.MidPoints
                    else
                        0
            ...
            Glucose, fun a v -> genCalc |> calcOne Glucose a v

            Potassium, fun a v -> genCalc |> calcOne Potassium a v

            Creatinine, fun a v -> genCalc |> calcOne Creatinine a v

            Urea, fun a v -> genCalc |> calcOne Urea a v

            WBC,
            fun a v ->
                fun v m -> if v < m.HighRange then m.HighPoints else 0
                |> calcOne WBC a v

            ProthPT, fun a v -> genCalc |> calcOne ProthPT a v

            ProthPTT, fun a v -> genCalc |> calcOne ProthPTT a v
    ...
    let calcItem a item v =
        ...
        |> function
            | Some c -> c |> snd |> (fun f -> f a v)
            | None -> $"no calculator for %A{item}" |> failwith
        ...
        | Some a ->
            ...
            | Neuro n1, NonNeuro n2 ->
                let s, sl =
                    ...
                    |> fun sl -> (sl |> List.map snd |> List.sum), sl

                s |> calcRiskFromScore |> Some, sl
            | _ ->
