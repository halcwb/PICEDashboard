...
module PIM =
    ...
    let pim2LowRisk =
        [ Asthma; Bronchiolitis; Croup; ObstructiveSleepApnea; DiabeticKetoacidosis ]
    // PIM3
    // Low-risk diagnosis:
    //  asthma,
    //  bronchiolitis,
    ...
    let calculatePIM2 (pim: PIM) =
        ...
        let paO2 = pim.PaO2 |> Option.defaultValue 0. |> kiloPascalToMmHg

        let bp = pim.SystolicBloodPressure |> Option.defaultValue 120.
        let be = pim.BaseExcess |> Option.defaultValue 0.
        let fiO2 = pim.FiO2 |> Option.defaultValue 0. |> (fun x -> x * 100.)
        ...
        let scoreList =
            [
                "elective", if pim.Urgency = Elective then -0.9282 else 0.

                "recovery", pim.Recovery |> recovScore

                "bypass", pim.Recovery |> byPassScore

                "low risk diagnosis",
                // lowRisc
                pim.RiskDiagnosis |> mapLowRisk |> Option.defaultValue 0.

                "high risk diagnosis", pim.RiskDiagnosis |> mapHighRisk |> Option.defaultValue 0.
                ...
                "ventilator", if pim.Ventilated then 1.3352 else 0.

                "SBP", Math.Abs(bp - 120.) * 0.01395

                "base excess", Math.Abs(be) * 0.1040

                "fiO2/paO2", if paO2 <= 0. then 0. else (fiO2 / paO2) * 0.2888

                "baseline", -4.8841
            ]

        let score, mort =
            let score = scoreList |> List.map snd |> List.sum
            score, calcRiskFromScore score
    ...
    let calculatePIM3 (pim: PIM) =
        ...
        let scoreList =
            [
                "elective", if pim.Urgency = Elective then -0.5378 else 0.
                ...
                "vent", if pim.Ventilated then 0.9763 else 0.

                "SBP", (-0.0431 * sbp) + (0.1716 * ((sbp ** 2.) / 1000.))

                "base excess", Math.Abs(be) * 0.0671
