...
let solutionFile = "PICEDashboard.sln"
...
let solutionRoot () =
    findParent __SOURCE_DIRECTORY__ solutionFile
...
let localConfig () =
    ...
    if isDevelopment then
        let configJsonPath =
            Path.Combine(solutionRoot (), "src/Server", "config.json")
...
let configureHost (builder: IHostBuilder) = builder |> configureVariables
