...
module Statistics =
    ...
    let periodInYear yr (from: DateTime option) (until: DateTime option) =
        if from > until then
            $"from: %A{from} cannot be later than until %A{until}" |> failwith
        else
        ...
        |> List.iter (fun yrTot ->
            yrTot.MonthTotals <-
                ...
                |> List.map (fun mo ->
                    ...
                    moTot.Month <- mo
                    moTot.Totals.Period <- mo |> intToMonth
    ...
    let calcPerc t n =
        try
            if t > 0 then
                ...
                $"%A{n}"
        with e ->
            $"error calcPerc %A{t} %A{n}\n%s{e.ToString()}" |> failwith
