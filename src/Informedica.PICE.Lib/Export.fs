...
module Export =
    ...
    let getPatientState (pa: PICUAdmission) (pat: Patient) =
        ...
        | Some dt1, Some dt2 ->
            ...
            | Some dt ->
                if dt >= dt1 && dt <= dt2 then
                    "Death"
                else
                    match pa.DischargeReason with
                    | None -> "Alive"
                    | Some dr -> if dr.Id = "100" then "Death" else "Alive"
            | None -> "Alive"
        | _ -> "Alive"
    ...
    let export (pats: Result<Patient[] * string[], _>) =
        ...
        |> List.collect (fun pat ->
            ...
            |> List.collect (fun ha ->
                ha.PICUAdmissions
                |> List.filter (fun pa -> pa.AdmissionDate.IsSome && pa.DischargeDate.IsSome)
                |> List.distinctBy (fun pa -> pa.HospitalNumber.Trim(), pa.AdmissionDate.Value)
                |> List.map (fun pa ->
                    let optPRISM p =
                        match p with
                        | None -> []
                    ...
                    [
                        ...
                        pa.Sepsis |> string
                        pa.PrimaryDiagnosis |> List.map _.Name |> String.concat ";"
                        pa.SecondaryDiagnosis |> List.map _.Name |> String.concat ";"
                        pa.Diagnoses |> List.map _.Name |> String.concat ";"
                        pa.ReferingSpecialism |> Option.map _.Label |> Option.defaultValue ""
                    ])))
        |> List.append headers
    ...
    let exportToDatabase (pats: Result<Patient[] * string[], _>) =
        ...
        |> function
            | Error _ -> ()
            | Ok _ ->
                let data = pats |> export |> List.distinctBy (fun x -> x[0], x[1])
                ...
                let headers =
                    ...
                    let toColumn s =
                        ...
                        | _ when s = "HospitalNumber" ->
                            {
                                Table.Name = s
                                Table.Type = Table.VarChar(100)
                                Table.IsKey = true
                            }
                        | _ when s = "AdmissionDate" ->
                            {
                                Table.Name = s
                                Table.Type = Table.DateTime
                                Table.IsKey = true
                            }
                        ...
                        | _ when stringCols |> List.exists ((=) s) ->
                            {
                                Table.Name = s
                                Table.Type = Table.VarChar(255)
                                Table.IsKey = false
                            }
                        | _ ->
                            {
                                Table.Name = s
                                Table.Type = Table.Float
                                Table.IsKey = false
                            }

                    data |> List.head |> List.map toColumn


                let db = Database.create serverName databaseName


                headers |> Table.createTable db.ConnectionString tableName |> ignore
