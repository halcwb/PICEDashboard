...
stats.YearTotals
|> List.iter (fun yr -> printfn $"{yr.Year}: {yr.Totals.Admissions}")
...
module Statistics =
    ...
    let periodInYear yr (from: DateTime option) (until: DateTime option) =
        if from > until then
            sprintf "from: %A cannot be later than until %A" from until |> failwith
        else
            match from, until with
            | None, _ -> false
            | Some fdt, None -> fdt.Year <= yr
            | Some fdt, Some udt -> fdt.Year <= yr && udt.Year >= yr
    ...
    let periodInYearMonth yr mo from until =
        if periodInYear yr from until |> not then
            false
        else
            match from, until with
            | None, _ -> false
            | Some adt, None -> adt.Month <= mo
            | Some adt, Some ddt -> adt.Month <= mo && ddt.Month >= mo


    let dateInYearMonth yr mo date =
        if dateInYear yr date |> not then
            false
        else
            match date with
            | None -> false
    ...
    let calculate (filter: Filter) (pats: Patient list) =
        ...
        let countBy un caps (ds: DataOption option list) =
            ...
            |> List.map (fun d ->
                ...
                | Some d -> d.Label
                | None -> un)
            |> List.map (fun s -> if caps |> List.exists ((=) s) then s else "Overige")
        ...
        let genderToCount (pats: Patient list) =
            pats
            |> List.distinctBy _.HospitalNumber
            |> List.map (fun p ->
                match p.Gender with
                | Male -> "Man"
                | Female -> "Vrouw"
            ...
            |> List.countBy id
            |> fun xs ->
                if xs |> List.exists (fst >> ((=) "Onbekend")) then
                    xs
                else
                    [ "Onbekend", 0 ] |> List.append xs
            |> fun xs ->
                if xs |> List.exists (fst >> ((=) "Man")) then
                    xs
                else
                    [ "Man", 0 ] |> List.append xs
            |> fun xs ->
                if xs |> List.exists (fst >> ((=) "Vrouw")) then
                    xs
                else
                    [ "Vrouw", 0 ] |> List.append xs
            |> List.sortBy (fun (k, _) ->
                match k with
                | s when s = "Man" -> 1
                | s when s = "Vrouw" -> 2
        ...
        let pats =
            ...
            pats
            |> List.filter (fun p -> notValid |> List.exists ((=) p) |> not)
            |> List.distinctBy _.HospitalNumber
            |> fun xs ->
                ...
                |> fun xs ->
                    ...
                    if c > 0 then
                        stats.Totals.InvalidPatients <-
                            match
                                stats.Totals.InvalidPatients
                                |> List.tryFind (fun (k, _) -> k = "Dubbele ziekenhuisopnames")
                            with
                            ...
                            | Some _ ->
                                ...
                                |> List.fold
                                    (fun acc (k, v) ->
                                        if k = "Dubbele ziekenhuisopnames" then
                                            (k, v + c) :: acc
                                        else
                                            (k, v) :: acc)
                                    []
                    ...
                    |> fun xs ->
                        ...
                        if c > 0 then
                            stats.Totals.InvalidPatients <-
                                match
                                    stats.Totals.InvalidPatients
                                    |> List.tryFind (fun (k, _) -> k = "Niet analyseerbare PICU opnames")
                                with
                                | None ->
                                    stats.Totals.InvalidPatients
                                    |> List.append [ "Niet analyseerbare PICU opnames", c ]
                                | Some _ ->
                                    stats.Totals.InvalidPatients
                                    |> List.fold
                                        (fun acc (k, v) ->
                                            if k = "Niet analyseerbare PICU opnames" then
                                                (k, v + c) :: acc
                                            else
                                                (k, v) :: acc)
                                        []
                    ...
                    let isOncology =
                        ...
                        || p.picuAdmission.ReferingSpecialism
                           |> function
                               | None -> false
                               | Some d -> d.Id = "30"
                        || (match p.picuAdmission.PRISM24 with
                            | Some prism -> prism.Cancer
                            | None -> false)

                    let isCardiac =
                        p.picuAdmission.PrimaryDiagnosis
                        |> List.exists (fun pd -> pd.Group = "cardiovasculair" || pd.Group = "hartchirurgie")
                        || p.picuAdmission.ReferingSpecialism
                           |> function
                               | None -> false
        ...
        stats.Totals.Patients <- pats |> List.map _.patient |> List.distinctBy _.HospitalNumber |> List.length

        stats.Totals.Admissions <- pats |> List.map _.picuAdmission |> List.length
        ...
        stats.Totals.DeathMode <-
            ...
            |> List.map _.patient
            |> List.filter Patient.patientPICUDeath
            |> List.map _.DeathMode
            |> fun xs -> xs |> countBy "Onbekend" (xs |> getCaps)

        stats.Totals.PIM2Mortality <-
            pats
            |> List.map _.picuAdmission
            |> List.filter (fun pa -> pa.PIM.PIM2Mortality |> Option.isSome)
        ...
        stats.Totals.PIM3Mortality <-
            pats
            |> List.map _.picuAdmission
            |> List.filter (fun pa -> pa.PIM.PIM3Mortality |> Option.isSome)
        ...
        stats.Totals.PRISM4Mortality <-
            pats
            |> List.map _.picuAdmission
            |> List.filter (fun pa -> pa |> getPRISMMort |> Option.isSome)
        ...
        stats.Totals.DischargeReasons <-
            pats
            |> List.map _.picuAdmission.DischargeReason
            |> fun xs -> xs |> countBy "Onbekend" (xs |> getCaps)

        stats.Totals.HospitalDischargeDestinations <-
            pats
            |> List.map _.hospitalAdmission.DischargeDestination
            |> fun xs -> xs |> countBy "Onbekend" (xs |> getCaps)

        stats.Totals.Urgency <-
            pats
            |> List.map _.picuAdmission.PIM.Urgency
            |> fun xs ->
                let data =
                    xs
                    |> List.map (fun x ->
                ...
                data |> List.countByList caps

        stats.Totals.Gender <- pats |> List.map _.patient |> genderToCount
        ...
        stats.Totals.Readmission <-
            pats
            |> List.map (fun p ->
                if p.picuAdmission.Readmission then
                    "Heropname"
                else
                    "Geen heropname")
        ...
        stats.Totals.Specialism <-
            pats
            |> List.map _.picuAdmission.ReferingSpecialism
            |> fun xs ->
                let xs = xs |> countBy "Onbekend" (xs |> getCaps)
                ...
                |> function
                    | _ when xs |> List.length > 10 ->
                        ...
                        |> List.fold
                            (fun acc (k, v) ->
                                if acc |> List.exists (fst >> (=) k) then
                                    acc
                                else
                                    acc
                                    |> List.map (fun (k', v') -> if k' = "Overige" then k', v' + v else (k', v')))
        ...
        stats.Totals.DiagnoseGroups <-
            pats
            |> List.collect (fun p -> p.picuAdmission.PrimaryDiagnosis |> List.map _.Group)
            |> List.countBy id
        ...
        stats.Totals.TransportTeam <-
            pats
            |> List.map _.hospitalAdmission.TransportTeam
            |> fun xs ->
                let caps = xs |> getCaps
                xs |> countBy "Onbekend" caps
        ...
        let inline filterAdmitted f m =
            ...
            |> List.filter (fun d -> f d.picuAdmission.AdmissionDate d.picuAdmission.DischargeDate)
            |> List.map m

        let inline filterAdmission f m =
            pats |> List.filter (fun p -> f p.picuAdmission.AdmissionDate) |> List.map m

        let inline filterDischarged f m =
            pats |> List.filter (fun p -> f p.picuAdmission.DischargeDate) |> List.map m
        ...
        |> List.iter (fun tot ->
            ...
            let admitted =
                filterAdmission (dateFilter yr None) _.patient
                |> List.distinctBy _.HospitalNumber

            tot.Totals.Patients <- admitted |> List.length

            tot.Totals.Deaths <- admitted |> List.filter Patient.patientAllTimeDeath |> List.length

            tot.Totals.Occupancy <- pats |> List.map _.picuAdmission |> calculateOccupancey tot.Year)
        ...
        |> List.iter (fun tot ->
            ...
            let admissions =
                filterAdmission (dateFilter yr None) _.picuAdmission
                |> List.filter _.DischargeDate.IsSome

            tot.Totals.Admissions <- admissions |> List.length
            ...
            tot.Totals.Gender <- filterAdmission (dateFilter yr None) id |> List.map _.patient |> genderToCount
            ...
            tot.Totals.DiagnoseGroups <-
                let grps = stats.Totals.DiagnoseGroups |> List.map fst
            ...
            tot.Totals.Diagnoses <-
                let dgs = stats.Totals.Diagnoses |> List.map fst
            ...
            tot.Totals.Readmission <-
                admissions
                |> List.map (fun pa -> if pa.Readmission then "Heropname" else "Geen heropname")
                |> List.countByList [ "Heropname"; "Geen heropname" ])
        // PICU discharge statistics
        yrTots
        |> List.iter (fun tot ->
            let yr = Some tot.Year
            let discharged = filterDischarged (dateFilter yr None) _.picuAdmission

            tot.Totals.Discharged <- discharged |> List.length

            tot.Totals.DischargeReasons <-
                discharged
                |> List.map _.DischargeReason
                |> fun xs -> xs |> countBy "Onbekend" (stats.Totals.DischargeReasons |> List.map fst)

            tot.Totals.HospitalDischargeDestinations <-
                filterDischarged (dateFilter yr None) _.hospitalAdmission
                |> List.map _.DischargeDestination
                |> countBy "Onbekend" (stats.Totals.HospitalDischargeDestinations |> List.map fst))
        // PICU admitted statistics
        yrTots
        |> List.iter (fun tot ->
            let yr = Some tot.Year
            let admitted = filterAdmitted (periodFilter yr None) _.picuAdmission

            tot.Totals.Admitted <- admitted |> List.length
        ...
        |> List.iter (fun yrTot ->
            yrTot.MonthTotals <-
                [ 1..12 ]
                |> List.filter (fun mo -> DateTime(yrTot.Year, mo, 1) > DateTime.Now |> not)
                |> List.map (fun mo ->
                    ...
                    moTot.Totals.Patients <- filterAdmitted (periodFilter yr mo) _.patient |> List.length

                    moTot.Totals.Deaths <-
                        filterAdmitted (periodFilter yr mo) _.patient
                        |> List.filter Patient.patientAllTimeDeath
                        |> List.length

                    moTot.Totals.PICUDeaths <-
                        filterAdmitted (periodFilter yr mo) _.picuAdmission
                        |> List.filter (fun pa -> pa.DischargeReason |> DataOption.EqsIdOpt "100")
                        |> List.length

                    moTot.Totals.Admissions <- filterAdmission (dateFilter yr mo) _.picuAdmission |> List.length

                    moTot.Totals.Admitted <- filterAdmitted (periodFilter yr mo) _.picuAdmission |> List.length

                    moTot.Totals.Discharged <- filterDischarged (dateFilter yr mo) _.picuAdmission |> List.length
                    ...
                    moTot.Totals.PICUDays <-
                        ...
                        |> List.map (fun pa ->
                            ...
                            let adt =
                                pa.AdmissionDate
                                |> Option.get
                                |> fun dt -> if dt.Month < moTot.Month then start else dt
                    ...
                    moTot.Totals.Gender <- filterAdmission (dateFilter yr mo) _.patient |> genderToCount
                    ...
                    moTot.Totals.DischargeReasons <-
                        filterDischarged (dateFilter yr mo) _.picuAdmission
                        |> List.map _.DischargeReason
                        |> fun xs -> xs |> countBy "Onbekend" (stats.Totals.DischargeReasons |> List.map fst)

                    moTot.Totals.TransportHospital <-
                        filterAdmission (dateFilter yr mo) _.hospitalAdmission
                        |> List.map _.TransportHospital
                        |> countBy "Onbekend" (stats.Totals.TransportHospital |> List.map fst |> List.distinct)
    ...
    let calcPerc t n =
        try
            if t > 0 then
                ...
                sprintf "%A" n
        with e ->
            sprintf "error calcPerc %A %A\n%s" t n (e.ToString()) |> failwith
    ...
    let printCount title kvs sort sb =
        ...
        |> fun sb ->
            let t = kvs |> List.map snd |> List.sum
            ...
            |> List.fold
                (fun acc (s, c) ->
                    ...
                    acc |> StringBuilder.appendLineFormat Literals.countItem [ s |> box; c |> box ])
                sb


    let countToTable tots get1 get2 sb =
        tots
        |> List.map (fun tot -> tot |> get1, tot |> get2)
        |> List.sortByDescending fst
        |> List.fold
            (fun acc (yr, xs) ->
                let calc c =
                    calcPerc (xs |> List.sumBy snd) (c |> float) |> box

                match acc with
                | [] ->
                    let acc = [ xs |> List.map (fst >> box) |> List.append [ "Periode" |> box ] ]
                    [ xs |> List.map (snd >> calc) |> List.append [ yr |> box ] ] |> List.append acc
                | _ -> [ xs |> List.map (snd >> calc) |> List.append [ yr |> box ] ] |> List.append acc)
            []
    ...
    let printMonthTabel (yrTot: YearTotals) sb =
        let caps =
            [ "Maand"; "Patienten"; "Opnames"; "Ontslagen"; "Ligdagen"; "Overleden" ]
            |> List.map box
        ...
        |> List.fold
            (fun acc stat ->
                ...
                acc |> StringBuilder.appendLineFormat Literals.columns6 vals)
            sb
    ...
    let toMarkdown (stats: Statistics) =
        ...
        |> StringBuilder.appendLineFormat
            Literals.dthTot
            [ calcPerc stats.Totals.Patients (float stats.Totals.PICUDeaths) |> box ]
        |> StringBuilder.appendLineFormat
            Literals.estPIM2
            [ calcPerc stats.Totals.Patients stats.Totals.PIM2Mortality |> box ]
        |> StringBuilder.appendLineFormat
            Literals.estPIM3
            [ calcPerc stats.Totals.Patients stats.Totals.PIM3Mortality |> box ]
        |> StringBuilder.appendLineFormat
            Literals.estPRISM
            [ calcPerc stats.Totals.Patients stats.Totals.PRISM4Mortality |> box ]
        |> StringBuilder.newLine2
        |> StringBuilder.appendLine "De getoonde mortaliteit in bovenstaande lijst is de PICU mortaliteit"
        |> StringBuilder.newLine2
        ...
        |> fun sb ->
            ...
            |> List.fold
                (fun acc stat ->
                    ...
                    acc |> StringBuilder.appendLineFormat Literals.columns9 vals)
                sb
            |> fun sb ->
                let t = stats.YearTotals |> List.sumBy _.Totals.Patients
                let calc = calcPerc t
                ...
                let vals =
                    [
                        ...
                        calc (stats.YearTotals |> List.sumBy _.Totals.PICUDeaths |> float) |> box
                ...
                sb |> StringBuilder.appendLineFormat Literals.columns9 vals
        |> StringBuilder.newLine
...
pats |> Array.toList |> Statistics.calculate Types.NoFilter
...
let temp filter pats =
    ...
    |> fun xs ->
        ...
        if c > 0 then
            stats.Totals.InvalidPatients <- stats.Totals.InvalidPatients |> List.append [ "Dubbele patienten", c ]
...
pats |> Array.toList |> temp Types.NoFilter
