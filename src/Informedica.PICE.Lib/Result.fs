...
module Result =
    ...
    let mergeMessages msgs result =
        let fOk (x, msgs2) = msgs @ msgs2 |> okMsg x
        let fErr errs = error (errs @ msgs)
    ...
    let bindR f result =
        let fOk (x, msgs) = f x |> mergeMessages msgs
        let fErr errs = Result.Error errs
        either fOk fErr result
    ...
    let applyR f result =
        match f, result with
        | Result.Ok(f, msgs1), Result.Ok(x, msgs2) -> msgs1 |> Array.append msgs2 |> okMsg (f x)
        | Result.Error msgs1, Result.Ok(_, msgs2)
        | Result.Ok(_, msgs2), Result.Error msgs1 -> msgs1 |> Array.append msgs2 |> error
        | Result.Error msgs1, Result.Error msgs2 -> msgs1 |> Array.append msgs2 |> error
    ...
    let mapMessagesR f result =
        match result with
        | Result.Ok(x, msgs) -> List.map f msgs |> okMsg x
        | Result.Error msgs -> List.map f msgs |> error
        ...
        |> Array.fold
            (fun acc result ->
                ...
                | Result.Ok(x, msgs1) ->
                    ...
                    | Result.Error _ -> acc
                | Result.Error msgs1 ->
                    let s = msgs1 |> String.concat "\n"
                    printfn $"found {msgs1 |> Array.length} errors:\n{s}\n"
    ...
    let foldOk2 results =
        let errs = System.Collections.Generic.List<string>()
        let mutable i = 0
        ...
        |> Array.collect (fun result ->
            ...
            | Error msgs ->
                errs.AddRange(msgs)
                [||])
        |> fun xs -> okMsg xs (errs |> Seq.toArray)
