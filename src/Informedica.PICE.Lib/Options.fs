...
module Options =
    ...
    let eqs (s1: String) (s2: String) =
        let canon (s: String) =
            s.ToLower().Trim().Replace("_", "").Replace("-", "")
    ...
    let codes =
        [|
            ...
            ("risicodiag-hoofd",
             "8",
             "Leverfalen, exclusief opnames na recovery levertransplantatie [HR PIM2 & VHR PIM3]")
            ...
            ("risicodiag-hoofd",
             "7",
             "Diabetische keto-acidose [LowRisk PIM2&3] (betekent ook 'acute diabtes' [PRISM3])")
            ("risicodiag-hoofd", "10", "Acute Diabetes [PRISM3]")
            ("risicodiag-hoofd",
             "11",
             "Niet-operatieve cardiovasculaire aandoeningen (bv: cardiomyopathie/myocarditis) [PRISM3]")
    ...
    let find name code =
        if String.IsNullOrWhiteSpace(name) || String.IsNullOrWhiteSpace code then
            None
        else
            codes
            |> Array.tryFind (fun (n, c, _) -> name |> eqs n && code |> eqs c)
            |> function
                | Some(_, id, label) -> Utils.DataOption.create id label |> Some
                | None -> None
